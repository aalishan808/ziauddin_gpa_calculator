   ...
   void _calculateGPA() {
     ...
     for (int i = 0; i < _numCourses; i++) {
       final course = Course(
         ...
         creditHours: int.tryParse(_creditHoursControllers[i].text) ?? -1, // Default to -1 for invalid input
         percentage: double.tryParse(_percentageControllers[i].text) ?? -1, // Default to -1 for invalid input
       );

       // Debug prints
       print('Course Name: ${course.name}, Credit Hours: ${course.creditHours}, Percentage: ${course.percentage}');

       // Add course only if valid
       if (course.creditHours > -1 && course.percentage >= -1 && course.percentage <=100) { 
         _viewModel.addCourse(course);
       } else {
         print('Invalid course data for Course ${i + 1}');
       }
     }
             ...
             children: [
               ...
               if (_showCourseInputs)
                 ElevatedButton(
                   ...
                   style :ElevatedButton.styleFrom(backgroundColor :Color(0xFF006c2c),padding :EdgeInsets.symmetric(horizontal :40, vertical :15),shape:RoundedRectangleBorder(borderRadius :BorderRadius.circular(10))),
                 ),
               ...
               if (_viewModel.courses.isNotEmpty)
                 Text('GPA:${_viewModel.calculateGPA().toStringAsFixed(2)}',style :TextStyle(fontSize :24,fontWeight:FontWeight.bold,color:Colors.white)),
             ],
           ),
         ),
       ),
